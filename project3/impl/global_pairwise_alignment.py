...
def global_pairwise_alignment(
    ...
    def compute(i: int, j: int) -> float:
        ...
        if i > 0 and j >= 0:
            vals[1] = compute(i - 1, j) + gap(1)
        if i >= 0 and j > 0:
            vals[2] = compute(i, j - 1) + gap(1)
        if i == 0 and j == 0:
            vals[3] = 0
        opt = min([v for v in vals if v is not None])
        dp[i][j] = opt
        return opt
    ...
    def backtrack(i: int, j: int, acc: dict[str, str], alignments: list[dict[str, str]]) -> list[dict[str, str]]:
        ...
        elif i >= 0 and j > 0 and curr == (left + gap(1)):
            copy = deepcopy(acc)
            copy['a'] += '-'
            copy['b'] += b[j]
            res += backtrack(i, j - 1, copy, alignments)
        elif i > 0 and j >= 0 and curr == (up + gap(1)):
            copy = deepcopy(acc)
            copy['a'] += a[i]
            copy['b'] += '-'
